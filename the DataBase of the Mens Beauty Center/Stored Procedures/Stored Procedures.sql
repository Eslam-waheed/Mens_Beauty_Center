...
-- ***3
CREATE or alter PROCEDURE SP_InsertEmployeeEvaluationWithPackages
    @Month NVARCHAR(10),
    @ProfitPercentage decimal(2,1)
AS
BEGIN
    -- Calculate the total amount of services provided by each employee for the given month
    WITH EmployeeServiceTotal AS (
        SELECT
            e.NationalID,
            SUM(s.Price) AS TotalServiceAmount
        FROM
            Serve sr JOIN Service s 
			ON sr.ServiceId = s.ID
            JOIN Employee e 
			ON sr.NationalID = e.NationalID
        WHERE
            FORMAT(sr.Servedate, 'yyyy-MM') =  @Month
        GROUP BY
            e.NationalID
    ),
    -- Calculate the total amount of packages taken by customers in the given month
    EmployeePackageTotal AS (
        SELECT
            e.NationalID,
            SUM(p.TotalAmount) AS TotalPackageAmount
        FROM
            PackageCustomer pc JOIN Package p 
			ON pc.PackageId = p.ID
            JOIN Employee e 
			ON e.NationalID = pc.NationalId -- Assuming CustomerId is linked with Employee (you may need to adjust this)
        WHERE
            FORMAT(pc.TakeDate, 'yyyy-MM') = @Month
        GROUP BY
            e.NationalID
    ),
    -- Combine both totals for each employee
    EmployeeTotal AS (
        SELECT
            COALESCE(es.NationalID, ep.NationalID) AS NationalID,
            COALESCE(es.TotalServiceAmount, 0) + COALESCE(ep.TotalPackageAmount, 0) AS TotalAmountOfMonth
        FROM
            EmployeeServiceTotal es FULL OUTER JOIN EmployeePackageTotal ep 
			ON es.NationalID = ep.NationalID
    )
    INSERT INTO Evaluation (Month, TotalAmountOfMonth, ProfitPercentage, NationalID)
    SELECT
        @Month,
        TotalAmountOfMonth,
        @ProfitPercentage,
...
END;
 ...
 SP_InsertEmployeeEvaluationWithPackages '2024-08', 5.0
-- *************************************************************************************************
-- Eslam
go
CREATE OR ALTER PROCEDURE PS_GetCustomerService
(
  @FromDate date,
  @ToDate date
)
AS
	BEGIN
	  ...
	  FROM
		serve sv
	  INNER JOIN
		Customer c ON sv.CustomerId = c.Code
	  INNER JOIN
		Employee e ON sv.NationalID = e.NationalID
	  INNER JOIN
		Service s ON sv.ServiceId = s.ID
	  WHERE
		sv.Servedate BETWEEN @FromDate AND @ToDate;
	END;
...
CREATE OR ALTER PROCEDURE SP_GetCustomerPackage
(
  @FromDate date,
  @ToDate date
)
AS
	BEGIN
	  SELECT
		...
		p.TotalAmount AS PackagePrice
	  FROM
		PackageCustomer pc
	  INNER JOIN
		Customer c ON pc.CustomerId = c.Code
	  INNER JOIN
		Employee e ON pc.NationalID = e.NationalID
	  INNER JOIN
		Package p ON pc.PackageId = p.ID
	  WHERE
		pc.TakeDate BETWEEN @FromDate AND @ToDate;
	END;


--EXEC PS_GetCustomerService '2023-08-01', '2025-12-22';  -- Retrieve data for Aug 1st to 22nd, 2024
--EXEC SP_GetCustomerPackage '2023-08-01', '2025-12-22';  -- Retrieve data for Jul 15th to Aug 20th, 2024 (adjust date field in WHERE clause)

go

CREATE OR ALTER PROCEDURE PS_GetCustomerServiceEmployee
(
  @FromDate date,
  @ToDate date,
  @NationalID char(14)
...
AS
	BEGIN
	  ...
	  INNER JOIN
		Service s ON sv.ServiceId = s.ID
	  WHERE
		sv.Servedate BETWEEN @FromDate AND @ToDate and e.NationalID = @NationalID;  -- Filter by date range
	END;
...
CREATE OR ALTER PROCEDURE SP_GetCustomerPackageEmployee
(
  @FromDate date,
  @ToDate date,
  @NationalID char(14)
...
AS
	BEGIN
	  ...
	  INNER JOIN
		Package p ON pc.PackageId = p.ID
	  WHERE
		pc.TakeDate BETWEEN @FromDate AND @ToDate and e.NationalID = @NationalID;
	END;

-- *************************************************************************************************

--EXEC PS_GetCustomerServiceEmployee '2023-08-01', '2025-12-22', '23456789012345';  -- Retrieve data for Aug 1st to 22nd, 2024
--EXEC SP_GetCustomerPackageEmployee '2023-08-01', '2025-12-22', '23456789012345';  -- Retrieve data for Jul 15th to Aug 20th, 2024 (adjust date field in WHERE clause)

-- *************************************************************************************************
-- Hosni
go

CREATE OR ALTER PROCEDURE SP_UpdateEmployeeInfo
    @FirstName NVARCHAR(50),
    @LastName NVARCHAR(50),
    @NationalID CHAR(14),
    @PhoneNumber VARCHAR(11),
